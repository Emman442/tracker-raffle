...
const InfoCard = ({ icon, title, value, unit }: { icon: React.ReactNode, title: string, value: string | number, unit?: string }) => (
  <motion.div
...
export default function LotteryStatus() {
  ...
  }, [lotteryData.endTime, lotteryData.status]);
  ...
  return (
    ...
    >
      ...
      <InfoCard 
        ...
        title={lotteryData.status === 'Open' ? 'Time Remaining' : 'Lottery Ended'}
        value={timeRemaining}
      />
    </motion.div>
  );
